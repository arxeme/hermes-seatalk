...
| W2-07 | 出站 account 选择与 Hermes patch | W2-02 | AFK | target parser、metadata priority、home channel、cron、send_message patch | account prefix 先解析；home/cron 返回 account-qualified target；内置平台行为不变 |
...
- `send_message`、home channel、cron 均支持 account selection。
...
目标：出站目标、home channel、cron 均能选择正确 account。
...
- `_patch_home_channel()` / `_patch_cron_scheduler()` 返回 account-qualified target。
...
- `get_chat_info("staging:EmpABC")` 使用 staging runtime client，不把 `EmpABC`
  误解析为 thread id。
- `home_channel_account_id` 生效。
- cron delivery 使用 account-qualified target。
...
目标：用户可通过 setup wizard 和 README 配置 accounts-only SeaTalk plugin。
...
- wizard 支持设置 home channel account。
- wizard 不写 `.env`，只写 `config.yaml` accounts。
- wizard 不展示 `pairing`。
- README 明确 `config.yaml` 包含 secrets。
- README 区分 `group_allow_from` raw group id 与 `home_channel` target 格式。
...
| home channel / cron account selection | W2-07 |
