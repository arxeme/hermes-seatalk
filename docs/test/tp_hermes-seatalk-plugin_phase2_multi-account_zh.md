...
### T2-10 OpenClaw 消息解析对齐

覆盖 WBS：W2-10

| 用例 | 类型 | 断言 |
| --- | --- | --- |
| T2-10-01 forwarded 媒体不丢失 | unit | 转发消息中含 image 时，`event.media_urls` 非空，`event.media_types` 含 `"image"` |
| T2-10-02 forwarded 发送者前缀 | unit | forwarded item 带 `sender.employee_code` 时，对应文本行以 `{sender_name}: ` 开头 |
| T2-10-03 forwarded 嵌套数组展开 | unit | `content` 中含 list 时，递归展开并包含其中 text 内容，不抛出异常 |
| T2-10-04 forwarded 嵌套 forwarded | unit | `content` 中含 `combined_forwarded_chat_history` item 时，递归解析，输出含 `[Forwarded messages]` 前缀 |
| T2-10-05 quoted 去重 同 buffer 两条消息引用同 id | unit | debounce buffer 中两条消息都引用同一 `quoted_message_id` 时，最终 `event.text` 中 quoted 文本只出现一次 |
| T2-10-06 quoted 不去重 不同 id | unit | buffer 中两条消息分别引用不同 `quoted_message_id` 时，两段 quoted 文本均出现 |
| T2-10-07 媒体下载重试成功 | unit | 媒体下载第一次失败（RuntimeError），第二次成功时，event.media_urls 非空，media_errors 为空 |
| T2-10-08 媒体下载两次均失败 | unit | 两次下载均失败时，event.media_urls 为空，media_errors 含错误信息（与现有 test_t06_06 行为一致） |
| T2-10-09 现有 quoted 测试回归 | unit | `test_t06_05_quoted_message` 用例继续通过，单条消息引用时 `event.text` 格式不变 |

### T2-11 SeaTalk 工具多帐号支持

覆盖 WBS：W2-11

| 用例 | 类型 | 断言 |
| --- | --- | --- |
| T2-11-01 schema 含 account_id 字段 | unit | `SEATALK_TOOL_SCHEMA["parameters"]["properties"]` 含 `account_id` 键，不在 `required` 中 |
| T2-11-02 account_id 传给 get_client | unit | handler 调用时将 `args["account_id"]` 传给可注入 get_client；get_client 接收到正确 account_id |
| T2-11-03 unknown account_id 返回 error | unit | get_client 收到 unknown account_id 返回 None 时，handler 返回含 `"error"` 的 JSON |
| T2-11-04 无 account_id 默认行为 | unit | 不传 account_id 时 handler 调用 get_client(account_id=None)，保持原有默认 account 行为 |
...
### Batch 7：OpenClaw 对齐与工具增强

覆盖：W2-10、W2-11

```bash
uv run pytest tests/test_w06_dispatcher.py tests/test_w11_seatalk_tools.py
```

### Batch 6：测试与回归收敛

覆盖：W2-09（含 W2-10、W2-11 全量回归）
